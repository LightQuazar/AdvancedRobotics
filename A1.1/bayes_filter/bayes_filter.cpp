...
void normaliseState(std::vector<double>& state)
{
  ...
  for (double& cellProbability : state)
    normalisingConstant += cellProbability;

  for (double& normalisedCell : state)
    normalisedCell /= normalisingConstant;  
}
...
void initialiseState(std::vector<double>& state)
{
  ...
  for (double& cellProbability : state)
    cellProbability = initialProbability;
}
